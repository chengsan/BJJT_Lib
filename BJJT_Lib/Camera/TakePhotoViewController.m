...
@interface TakePhotoViewController ()<RSKImageCropViewControllerDelegate>
...
- (void) initialSession
{
    ...
    //stadicView默认不隐藏
    if ([_currntMark isEqualToString:@"1"]) {
        _stadicView.hidden = YES;
    }
    
    //warnLabel2是否不隐藏
    if ([self.hidesWarnLabel2 isEqualToString:@"1"]) {
        _warnLabel2.hidden = NO;
    }
    //staticView是否隐藏
    if ([self.hidesStaticView isEqualToString:@"1"]) {
        _stadicView.hidden = YES;
    }
    ...
    _warnLabel2.transform = CGAffineTransformMakeRotation(M_PI/2);
...
- (void) shutterCamera
{
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
       ...
       // NSLog(@"image size = %@",NSStringFromCGSize(image.size));
        ...
        if (_photoType == 1)
        {
            RSKImageCropViewController *imageCropVC = [[RSKImageCropViewController alloc] initWithImage:_photoImage  cropMode:RSKImageCropModeCircle];
